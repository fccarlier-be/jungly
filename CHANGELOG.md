...
- **Photo cassée apres identification** : constate en staging sur des plantes creees via identification par photo, persistant apres un premier correctif partiel (miniature du formulaire uniquement). Cause reelle, confirmee par une regression documentee de Next.js ([vercel/next.js#82610](https://github.com/vercel/next.js/issues/82610)) : l'optimiseur d'images (`/_next/image`) fait un fetch interne SANS jamais transmettre les cookies de la requete d'origine -- inoffensif pour une image publique, mais `/uploads/[filename]` exige une session (photo privee, voir `src/app/uploads/[filename]/route.ts`), donc cet appel echoue systematiquement et Next.js met l'echec en cache, laissant la photo cassee durablement. Affecte tout affichage d'une photo `/uploads/...`, pas seulement l'identification par photo -- simplement plus vite atteint par ce nouveau flux. **`src/lib/imageOptimization.ts`** (nouveau) : `bypassesImageOptimizer()`, appliquee partout ou une photo `/uploads/...` s'affiche (`PlantForm`, `PlantCard`, `PlantCoverPhoto`, `PlantPhotoGallery`, `PlantPhotoLightbox`, tableau de bord, `TaskCard`) -- `/library-photos/...` (public, non concerne) reste optimisable.
- **Photo toujours cassee dans le formulaire de creation** (`src/app/uploads/[filename]/route.ts`) : meme en contournant l'optimiseur, la miniature restait 404 -- le fichier n'est pas encore rattache a une plante a ce stade, et la route ne reconnaissait comme proprietaire que via une plante existante. Deja documente comme acceptable pour un televersement manuel classique (miniature peu visible), mais trop genant avec l'identification par photo. La ligne `Upload` (creee des le televersement) suffit desormais a prouver que le compte connecte est bien l'auteur du fichier, sans attendre qu'il soit attache a quoi que ce soit.
...
- **Lifecycle de la table `Upload` incohérent dans les deux sens** : si `db.upload.create()` échouait après un `writeFile()` réussi, le fichier physique restait orphelin (jamais nettoyé) ; inversement, `deleteUploadedFile()` ne supprimait jamais la ligne `Upload` correspondante. `POST /api/uploads` nettoie désormais le fichier si l'insertion DB échoue ; `deleteUploadedFile()` supprime aussi la ligne `Upload` après un `unlink()` réussi (silencieux si absente -- fichier importé via ZIP, jamais associé à un `Upload`). Vérifié en direct : upload → ligne présente → suppression de la plante → fichier et ligne `Upload` disparus ensemble.
- **`GET /api/plants/:id/history` sans plafond** : une plante ancienne (ou restaurée via import, jusqu'à 10 000 événements) pouvait produire une réponse très volumineuse -- `take: 200` par défaut, paramètre `limit` borné à 500. Route API dormante (aucun composant du frontend ne l'appelle actuellement, la fiche plante interroge `careEvent` directement avec `take: 5`), pas de pagination curseur ajoutée tant qu'aucun consommateur réel n'en a besoin.
- **PATCH d'une règle de soin ne validait pas le contenu de `configuration`** : contrairement à `POST /api/care-rules` (discriminated union par type), `PATCH /api/care-rules/:id` acceptait `configuration` comme un `z.record` non typé -- des valeurs incohérentes (`dosagePerLiter` négatif, clés arbitraires) passaient sans contrôle. Nouvelle map `configSchemaByType` (réutilise les schémas déjà définis pour CREATE, désormais `.strict()` pour rejeter aussi les clés inconnues -- CREATE en profite au passage) : le PATCH revalide `configuration` avec le schéma du type existant (immuable sur un PATCH) quand il est explicitement fourni. Vérifié en direct : dosage négatif → 400, clé arbitraire → 400, configuration valide → 200, PATCH ne touchant pas `configuration` → toujours 200 (pas de régression).
- **CareEngine : `completeTaskWithEvent`/`recordStandaloneCareEvent` pas pleinement atomiques** : la création du `CareEvent`, le passage de la tâche à `COMPLETED` et la génération de la prochaine échéance (`ensurePendingTaskForRule`) se faisaient en deux étapes séparées -- un échec de la seconde (ex. panne DB) pouvait laisser un historique complété sans que la règle récurrente ne produise sa suivante. Même problème pour `repot` : l'événement et la mise à jour du pot/substrat de la plante étaient deux appels distincts. Les deux fonctions englobent désormais tout dans une seule transaction (`recordStandaloneCareEvent` suit le même schéma que `ensurePendingTaskForRule` : accepte un client de transaction optionnel, réutilisé par `repot` pour y inclure `db.plant.update`). Vérifié en direct : complétion de tâche → nouvelle échéance générée correctement ; rempotage → événement créé et pot/substrat mis à jour en un seul appel.
- **Import/export sans rate-limit** : contrairement aux créations individuelles (audit4), `POST /api/import` (jusqu'à 1000 plantes/100 capteurs/50 Mio décompressés par appel) et `GET /api/export` (lecture de tous les readings/photos + compression ZIP) n'avaient aucune limite -- un usage répété en boucle faisait travailler Prisma/SQLite/le filesystem/Sharp/JSZip sans contrainte. `checkRateLimit` : 3 imports/heure, 5 exports/heure, par utilisateur. Vérifié en direct : 4e import → 429, 6e export → 429.
- **`MAX_ZIP_ENTRIES` ne comptait que les entrées `uploads/*`** : une archive pouvait contenir des milliers d'entrées hors de ce préfixe (répertoires/fichiers vides, jamais décompressés par le code), toutes chargées en mémoire dès `JSZip.loadAsync()`, sans jamais dépasser `MAX_ZIP_FILE_SIZE` (10 Mio). Contrôle déplacé sur `Object.values(zip.files).length` (toutes entrées), avant tout traitement du contenu -- le contrôle spécifique à `uploads/*` devenu redondant a été retiré. Vérifié en direct : archive de 280 Ko avec 3000 entrées vides → 400 immédiat.
...
- **Politique de rétention des lectures de capteurs** : aucune limite n'existait -- un capteur légitime à 1 mesure/minute produit ~525 600 lignes/an, SQLite/Prisma en souffrent bien avant que ce chiffre devienne confortable. `tickSensorRetention()` (même schéma que `tickWeather()` dans `scheduler.ts`, vérification une fois par jour) purge les `SensorReading` de plus de 90 jours, sans downsampling/agrégation (sur-ingénierie tant qu'aucun capteur réel n'est branché). Vérifié en direct : lecture de 100 jours purgée, lecture de 10 jours conservée.
- **Quotas de base sur la création de ressources** : `POST /api/plants`, `/api/notes`, `/api/fertilizers`, `/api/locations` n'avaient aucune limite (contrairement aux opérations déjà protégées : uploads, capteurs, recherche externe, connexion). `checkRateLimit` par utilisateur (50/h plantes, engrais, emplacements ; 100/h notes), même utilitaire que partout ailleurs.
- **Import de bibliothèque externe réservé à l'admin** : `POST /api/library/import` écrivait dans `PlantLibraryEntry` (table globale partagée) depuis n'importe quel compte authentifié. Réservé à l'admin (`requireAdminUserId`, même motif que `/resync`). Flux préservé pour un compte normal : `session.user.isAdmin` exposé (nouveau champ sur `User`/JWT/session, même schéma que `id`) ; `ExternalSpeciesSearch.tsx` n'appelle plus l'API d'import pour un non-admin, utilise directement les données déjà récupérées pour l'aperçu (aucun appel réseau de plus) pour pré-remplir SA plante, sans créer de fiche partagée (`libraryEntryId` reste `null`, `PlantForm.applyLibraryEntry` ne le définit que si un id réel est fourni). Vérifié en direct : 403 pour un compte non-admin, l'admin passe le contrôle et atteint l'API externe. La suite E2E `auth.spec.ts` n'a pas pu être rejouée proprement une dernière fois (rate limiter de connexion/inscription épuisé par les vérifications répétées de cette session, conteneur Playwright complet ayant par ailleurs provoqué plusieurs arrêts mémoire sur cet hôte partagé) -- la même logique serveur (`authorize`/`jwt`/`session`) a cependant déjà été exercée avec succès par 6 connexions distinctes dans un run E2E complet antérieur (9/10, seul l'échec restant confirmé comme du rate limiting via curl direct avec un compte fraîchement créé).
- **`PushSubscription` : un `endpoint` connu permettait de se l'approprier silencieusement** : `upsert({ update: { userId, ... } })` réécrivait le propriétaire sans vérifier qui possédait déjà l'abonnement. Vérifie désormais explicitement avant l'upsert -- 409 si l'`endpoint` appartient à un autre compte. Vérifié en direct : compte B tente de reprendre l'endpoint de A → 409, l'abonnement reste à A.
- **`middleware.ts` : défense en profondeur** `if (!req.auth)` → `if (!req.auth?.user)`, pattern recommandé par l'advisory NextAuth GHSA-8fpg-xm3f-6cx3 -- `beta.32` corrige déjà le fail-open côté librairie, sécurité supplémentaire si ce comportement revenait.
